    ...
    return (
        <div className="flex justify-center mt-7">
            {isLoading ? <div>데이터를 불러오는 중...</div> : (
                <div className="flex flex-col w-4/6 p-3 space-y-4 rounded-lg bg-slate-300">
                    {data?.slice(0, 100).map((coin) => (
                            <Link to={coin.id} key={coin.id} state={{coninId: coin.id}}>
                                <div className="flex items-center p-3 rounded-lg bg-slate-400">
                                    <img
